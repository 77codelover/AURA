...
export class HandTrackingService{
 ...
 async init(){const vision=await FilesetResolver.forVisionTasks('https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@latest/wasm'),options=(delegate:'GPU'|'CPU')=>({baseOptions:{modelAssetPath:'https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task',delegate},runningMode:'VIDEO' as const,numHands:2,minHandDetectionConfidence:.5,minTrackingConfidence:.45});try{this.landmarker=await HandLandmarker.createFromOptions(vision,options('GPU'))}catch{this.landmarker=await HandLandmarker.createFromOptions(vision,options('CPU'))}}
